...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        // Sticky Header - replaced with CSS sticky, no JS clone needed
        // (sticky behavior handled by CSS position:sticky on .bottom-header)

        // Mobile Menu
        function mobileMenu() {
            $("header.main-header").after(
                '<div class="mobile-navigation-menu"><button id="mobile-menu-close"><svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24" fill="currentColor"><path d="M256-213.847 213.847-256l224-224-224-224L256-746.153l224 224 224-224L746.153-704l-224 224 224 224L704-213.847l-224-224-224 224Z"/></svg></button></div>',
            );
            var menuWrapper = $(
                "header.main-header .header-menu-wrap .nav-menu",
            ).clone();
            $(".mobile-navigation-menu #mobile-menu-close").after(menuWrapper);

            $("#mobile-menu-close, .mobile-menu-action").on(
                "click",
                function () {
                    $(".mobile-navigation-menu").toggleClass(
                        "open-mobile-menu",
                    );
                },
            );
            ...
            $(".mobile-navigation-menu .dropdown-plus").on(
                "click",
                function () {
                    $(this).prev("ul").slideToggle(300);
                    $(this).toggleClass("dropdown-open");
                    $(".mobile-navigation-menu ul li:has(ul)").toggleClass(
                        "dropdown-open",
                    );
                },
            );

            $(".mobile-navigation-menu .dropdown_menu a").append(
                "<span></span>",
            );
        }
        ...
        // Menu Text Animation
        document
            .querySelectorAll(".main-header-wapper .nav-menu > li > a")
            .forEach(
                (btnText) =>
                    (btnText.innerHTML =
                        "<span>" +
                        btnText.textContent.split("").join("</span><span>") +
                        "</span>"),
            );

        // Button Text Animation
        document
            .querySelectorAll(".text-anim")
            .forEach(
                (btnText) =>
                    (btnText.innerHTML =
                        "<span>" +
